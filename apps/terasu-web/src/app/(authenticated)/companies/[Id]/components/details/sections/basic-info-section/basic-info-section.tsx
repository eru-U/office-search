...
export const BasicInfoSection = ({ data, onRefresh }: Props) => (
  <section className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <InfoBox
        label="設立年月日"
        value={data.company.establishedDate ? data.company.establishedDate.toLocaleDateString() : "未登録"}
      />
      <InfoBox
        label="資本金"
        value={data.company.capital ? `${data.company.capital.toLocaleString()} 円` : "未登録"}
      />
      <InfoBox label="代表者名" value={data.representative || "未登録"} />
      <InfoBox
        label="社員数"
        value={data.employeeCount ? `${data.employeeCount.toLocaleString()} 名` : "未登録"}
      />
